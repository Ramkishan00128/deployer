...
app.post("/deploy-bigeyes", async (req, res) => {
  ...
  const contractTemplate = fs.readFileSync(
    `./Templates/${templateName}.sol`,
    "utf8"
  );

  // Replace the placeholder with the user-provided name in the contract template
  const updatedContractSource = contractTemplate.replace(
    "CONTRACTNAME",
    contractName
  );
  ...
  if (updatedContractSource === contractTemplate) {
    return res.status(400).json({
      error: "Placeholder 'CONTRACTNAME' not found in the contract template.",
    });
  }

  // Write the updated contract to a temporary file
  fs.writeFileSync(
    `./Generated/${contractName}.sol`,
    updatedContractSource,
    "utf8"
  );
  ...
  const contractAbi = JSON.parse(
    output.contracts[`${contractName}.sol`][`${contractName}`].metadata
  ).output.abi;
  ...
  fs.writeFileSync(
    path.join(artifactsFolderPath, `${contractName}.json`),
...
app.post("/deploy-neocypherpunk", async (req, res) => {
  const { contractName, templateName, router, name, symbol, privateKey } =
    req.body;
...
app.post("/deploy-pepetoken", async (req, res) => {
  const { contractName, templateName, name, symbol, totalSupply, privateKey } =
    req.body;
